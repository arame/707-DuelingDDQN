...
def plot_learning_curve(x, scores, epsilons):
    ...
    colour_blue = "C0"
    ax.plot(x, epsilons, color=colour_blue)
    ax.set_xlabel("Training Steps", color=colour_blue)
    ax.set_ylabel("Epsilon", color=colour_blue)
    ax.tick_params(axis='x', colors=colour_blue)
    ax.tick_params(axis='y', colors=colour_blue)
    ...
    colour_orange = "C1"
    ax2.scatter(x, running_avg, color=colour_orange)
    ax2.axes.get_xaxis().set_visible(False)
    ax2.yaxis.tick_right()
    ax2.set_ylabel('Score', color=colour_orange)
    ax2.yaxis.set_label_position('right')
    ax2.tick_params(axis='y', colors=colour_orange)
